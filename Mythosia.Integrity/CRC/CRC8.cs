...
namespace Mythosia.Integrity.CRC
{
    ...
    public class CRC8 : ErrorDetection
    {
        ...
        public override IEnumerable<byte> Compute(IEnumerable<byte> source)
        {
            ...
            if (Type == CRC8Type.Basic) result.Add(ComputeClassicCRC8(source));
            else if (Type == CRC8Type.Classic) result.Add(ComputeClassicCRC8(source));
            else if (Type == CRC8Type.Maxim) result.Add(ComputeMaximCRC8(source));
        ...
        {
            if (sourceWithCRC.Count() <= 1) return true;

            return sourceWithCRC.Last() != sourceWithCRC.Take(sourceWithCRC.Count() - 1).CRC8(Type);
        }
        ...
        private static readonly byte[] _crc8ClassicTable = // CCITT?
{
        ...
        private byte ComputeClassicCRC8(IEnumerable<byte> data)
        {
        ...
        private static byte ComputeMaximCRC8(IEnumerable<byte> data)
        {
            if (data.Count() == 0) return 0;
