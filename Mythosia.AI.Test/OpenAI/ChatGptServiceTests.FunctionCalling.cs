...
namespace Mythosia.AI.Tests
{
    public partial class ChatGptServiceTests
    {
        ...
        public async Task BasicFunctionCallingTest()
        {
            try
            {
                // Function 등록 - 올바른 오버로드 사용
                AI.WithFunction(
                    "get_current_time",
                    "Gets the current time in a specified timezone",
                    (Dictionary<string, object> args) =>
                    {
                        var timezone = args.ContainsKey("timezone") ? args["timezone"].ToString() : "UTC";
                        var time = timezone == "UTC" ? DateTime.UtcNow : DateTime.Now;
                ...
                // Function이 호출되었는지 확인
                var lastMessage = AI.ActivateChat.Messages.LastOrDefault(m => m.Role == ActorRole.Function);
                if (lastMessage != null)
                {
                    Console.WriteLine($"[Function Called] Result: {lastMessage.Content}");
                    Assert.IsTrue(lastMessage.Content.Contains("Current time"));
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task FunctionCallModeTest()
        {
            try
            {
                ...
                AI.WithFunction(
                    "test_function",
                    "A test function",
                    (Dictionary<string, object> args) => "Function was called!");
        ...
        public async Task DisableFunctionsTest()
        {
            try
            {
                ...
                AI.WithFunction(
                    ...
                    (Dictionary<string, object> args) => "This shouldn't be called when disabled!");
            ...
            try
            {
                ...
                AI.WithFunction(
                    ...
                    (Dictionary<string, object> args) =>
                    {
                        ...
                        var coords = city.ToLower() switch
                        {
                            "seoul" => new { lat = 37.5665, lon = 126.9780 },
                            "tokyo" => new { lat = 35.6762, lon = 139.6503 },
                ...
                // 체인 실행을 유도하는 프롬프트
                var response = await AI.GetCompletionAsync(
                    "Get the weather for Seoul by first finding its coordinates");
                ...
                Console.WriteLine($"[Function Calls] Total: {functionMessages.Count}");
                foreach (var msg in functionMessages)
                {
                    Console.WriteLine($"  - {msg.Content.Substring(0, Math.Min(100, msg.Content.Length))}...");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[Function Chaining Error] {ex.Message}");
                Assert.Fail(ex.Message);
            }
        }
        ...
        public async Task FunctionErrorHandlingTest()
        {
            try
            {
                // 에러를 발생시키는 Function 등록
                AI.WithFunction(
                    "error_function",
                    "A function that throws an error",
                    (Dictionary<string, object> args) =>
                    {
                        if (args.ContainsKey("force_error") &&
                            bool.TryParse(args["force_error"].ToString(), out var forceError) &&
                            forceError)
                        {
                ...
                // 에러를 발생시키는 호출
                var response = await AI.GetCompletionAsync(
                    "Call error_function with force_error set to true");

                Assert.IsNotNull(response);
                Console.WriteLine($"[Error Handling] {response}");
        ...
        public async Task FunctionExtensionMethodsTest()
        {
            try
            {
                ...
                AI.WithFunction(
                    "test_func",
                    "Test function",
                    (Dictionary<string, object> args) => "Function executed!");
